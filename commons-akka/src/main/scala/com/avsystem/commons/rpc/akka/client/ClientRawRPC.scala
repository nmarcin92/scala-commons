...
private[akka] final class ClientRawRPC(config: AkkaRPCClientConfig, getterChain: Seq[RawInvocation] = Nil)(implicit system: ActorSystem) extends AkkaRPCFramework.RawRPC {
  ...
  override def fire(rpcName: String, args: BMap[String, AkkaRPCFramework.RawValue]): Unit = {
    system.actorSelection(config.serverPath) ! ProcedureInvocationMessage(rpcName, args, getterChain)
  }
  override def call(rpcName: String, args: BMap[String, AkkaRPCFramework.RawValue]): Future[AkkaRPCFramework.RawValue] = {
  ...
  override def get(rpcName: String, args: BMap[String, AkkaRPCFramework.RawValue]): RawRPC =
    new ClientRawRPC(config, getterChain :+ RawInvocation(rpcName, args))

  override def observe(rpcName: String, args: BMap[String, AkkaRPCFramework.RawValue]): Observable[AkkaRPCFramework.RawValue] = {
