...
private final class ServerActor(rawRPC: AkkaRPCFramework.RawRPC, config: AkkaRPCServerConfig) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case msg@ProcedureInvocationMessage(name, args, getterChain) =>
      resolveRpc(msg).fire(name, args)
    case msg@FunctionInvocationMessage(name, args, getterChain) =>
      import com.avsystem.commons.concurrent.RunNowEC.Implicits.executionContext
      val s = sender()
      resolveRpc(msg).call(name, args).onComplete {
        case Success(value) => s ! InvocationSuccess(value)
        case Failure(e) =>
          logError(e, name)
          s ! InvocationFailure(e.getClass.getCanonicalName, e.getMessage)
      }
    case msg@ObservableInvocationMessage(name, args, getterChain) =>
      ...
      resolveRpc(msg).observe(name, args).subscribe(
        value => {
  ...
  private def resolveRpc(msg: InvocationMessage) = rawRPC.resolveGetterChain(msg.getterChain.map(r => AkkaRPCFramework.RawInvocation(r.rpcName, r.args)).toList)
